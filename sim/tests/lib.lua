...
function transmit_rom(filename, baseaddress, transmit_dwords)
   ...
   reg_set(#filecontent_char, swan.Reg_swanA_MaxPakAddr)
...
function write_gbbus_32bit(data, address)
   ...
   reg_set_connection(data, swan.Reg_swanA_BusWriteData)
   reg_set_connection(address + 0x40000000, swan.Reg_swanA_BusAddr) 
   
end
function read_gbbus_32bit(address)

   reg_set_connection(address + 0x50000000, swan.Reg_swanA_BusAddr)
   return reg_get(swan.Reg_swanA_BusReadData)
   
end
function compare_gbbus_32bit(comparevalue, address)

   reg_set_connection(address + 0x50000000, swan.Reg_swanA_BusAddr)
   local value = reg_get(swan.Reg_swanA_BusReadData)
...
function write_gbbus_16bit(data, address)
   ...
   reg_set_connection(data, swan.Reg_swanA_BusWriteData)
   reg_set_connection(address + 0x20000000, swan.Reg_swanA_BusAddr) 
   
end
function read_gbbus_16bit(address)

   reg_set_connection(address + 0x30000000, swan.Reg_swanA_BusAddr)
   return reg_get(swan.Reg_swanA_BusReadData)
...
function write_gbbus_8bit(data, address)
   ...
   reg_set_connection(data, swan.Reg_swanA_BusWriteData)
   reg_set_connection(address + 0x00000000, swan.Reg_swanA_BusAddr) 
   
end
function read_gbbus_8bit(address)

   reg_set_connection(address + 0x10000000, swan.Reg_swanA_BusAddr)
   return reg_get(swan.Reg_swanA_BusReadData)
